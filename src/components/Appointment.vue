...
<template>
  <div class="bg-white p-5 space-y-3 rounded-lg">
    ...
    <p class="text-2xl font-black text-right">
      Total a pagar: <span class="text-blue-500">{{ formatCurrency(appointment.total) }}</span>
    </p>
    <div class="flex gap-2 items-center">
      <RouterLink
        class="bg-slate-600 rounded-lg p-3 text-white text-sm font-black uppercase flex-1 md:flex-none"
        :to="{ name: 'edit-appointment', params: { id: appointment._id } }">
        Editar Cita
      </RouterLink>
      <button
        class="bg-red-600 rounded-lg p-3 text-white text-sm font-black uppercase flex-1 md:flex-none"
